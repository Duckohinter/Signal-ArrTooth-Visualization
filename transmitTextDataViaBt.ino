  ...
  int T = 500; //delay in ms between data transmissions which gives time for the app to plot data. T can be lowered if more sensors are plugged in since it takes about 0.1 ms to analogRead(pin) 
  int i;
  byte delim =0;
...
void sendTextMessage(){
  Serial.print('<'); //< > used to set apart different arrays
  Serial.print(messageToApp);  
  Serial.print('>');
  Serial.write(delim);
}
